...
2. Add or rewrite the row below: gallery page, still, one line of what
   the idle first screen must show (including locale: fill origin,
   digits, start/end). Theme and Colors are extra gallery pages, not
   catalog ids. SCORE `visual-map` fails without the row or
   the PNG.
...
| type | `book/src/images/content.png` | `label` + `LabelFace` (Body / Meta / Display / Figure) + icon grid above the fold. RTL: page-title labels and the icon pane on the start. Locale: the mono sample is catalog fill (xem Mã / コードページ / 代码页), not leftover English `Code`. Icon-grid slugs (`close`, `chevron`) are the copy-name job, not leftover English |
| markdown | `book/src/images/content.png` | Full document via `markdown_view` + `MarkdownOpts` outline, not a one-line stub. RTL: outline on the start, document after it. Locale: document title is catalog fill (マークダウン / 标记 / ماركداون / مارک ڈاؤن / מרקדאון), not leftover Latin `Markdown`. vi keeps the loanword `Markdown` (same as `page.markdown`) — that is catalog fill |
| code | `book/src/images/content.png` | `highlighted_code` multi-line source (`syntax` set). `None` is plain monospace |
...
| list | `book/src/images/collections.png` | Virtual column + list with search/filters/pagination on the first screen. No Field-radius ring around the virtual column or the list pane — card/row chrome is the item face. A stadium wrapping two cards is broken. Page well and filled group have no outline; idle list/virtual-column cards keep their own outline; selected card is a wash, not a 2 dp primary frame. Catalog Search: glass and placeholder share the bar midline; leftover-high placeholder next to a centered glass is broken. RTL: rail on the end; ar/ur range digits Eastern. Locale: expand-card hint is catalog fill, not leftover `virtual_column` |
| log | `book/src/images/collections.png` | Virtualized lines; rail on the end in RTL |
| grid | `book/src/images/collections.png` | Shared-height tiles. No ring around the whole tile wall; selected tile is tonal, idle tiles outlined. A second outline wrapping the grid is broken. After `grid 2`: status/job note is the locale Tile word plus mapped digits (Tile 2 / Ô 2 / タイル 2 / 格 2 / بلاطة ٢ / ٹائل ٢ / אריח 2), never leftover `Opened tile 2` |
...
| theme | (page still; extra gallery page, not a catalog id; light beat is the paper canvas) | Named colorway tiles from `theme::named`; light beat is light. Locale: family hint is catalog fill (系統 / 系列 / Nhóm), not leftover Latin `Family`. RTL: first tile (dark) on the start (right); wrap rows start-align. Look-strip and tile labels `dark` / `light` are registered colorway ids, not leftover English |
| colors | `book/src/images/chrome.png` | Extra gallery page (not a catalog id). Token washes from `theme::mix`, not one-off hex. Locale: token labels are catalog fill (悬停 / ホバー / تحويم). vi keeps the role slugs `hover` / `pressed` / `chip` — that is catalog fill, not a missing row |
| keys | `book/src/images/chrome.png` | Cheatsheet catalog host plus the type-a-key pad: title, a checkbox, a list, and a status bar. Locale: the type-a-key hint is catalog fill (エンター / إدخال / داخلہ / אנטר), not leftover Latin `Enter` / `Escape`. RTL: Type-a-key title and hint under the heading on the start (right), not across the gutter; checkbox and list on the start; status chords on the end. Locale density matches English |
...
| feedback | `book/src/images/chrome.png` | Busy overlay + toast + `widget::scroll` (`ThemedScroll`, 24 px rail, not iced's 2 px scroller). Toast has a Level 3 drop. Rail on the end in RTL. Scroll lines start-align (right in RTL), not flush to the rail |
| dialogs | `book/src/images/patterns.png` | Side sheet and `dialog_sheet` confirm on a dim wash (`DialogOpts` backdrop). The confirm card stays centered on that wash (modal) — start-aligning it is wrong. Locale: every dialog action label is fully visible (sheet fills the caller width). Filename `notes.txt` is an LTR island, not leftover English. RTL: Open-sheet action on the start; after `sheet true` the sheet docks on the end (left) with close on the sheet end, not physical right |
...
| inspector | `book/src/images/patterns.png` | Tree, body, properties. Trailing badges sit on the end, clear of the filename (`introduction.md` must not paint through `md`). RTL: tree on the start (right), properties on the end; property labels start-align inside that pane. Job names the properties pane, not a physical side |
...
| motion | `book/src/images/motion.png` | Three tiles: Opens, Switch, then Attention stacked with Expand. Opens shows the duration slider and Open (overlay closed on idle). RTL: Opens on the start (right). Locale: pick labels and Start/End hint are catalog fill |
