...
| keys | `book/src/images/chrome.png` | Cheatsheet list. Locale: the type-a-key hint is catalog fill (エンター / إدخال / داخلہ / אנטר), not leftover Latin `Enter` / `Escape`. RTL: Type-a-key title and hint under the heading on the start (right), not across the gutter; action title on the start, shortcut on the end; rail gutter on the end. Locale density matches English |
...
| chrome-rows | `book/src/images/chrome.png` | Menu, toolbar, status. RTL: titles one line; action order from start. Status key chords (`ctrl+n`) stay LTR, not leftover English |
