...
const ROWS: &[(&str, &str, &str, &str, &str)] = &[
    ...
    ("pal.hint", "Type to filter. Nested Theme keeps the query. Ask adds a field under it.", "Gõ để lọc. Theme lồng giữ truy vấn. Hỏi thêm một ô bên dưới.", "打って絞る。入れ子の Theme はクエリを残す。Ask はその下に欄を足す。", "输入以过滤。嵌套 Theme 保留查询。Ask 在其下加一栏。"),
    ...
    ("field.form-hint", "Tab and Shift+Tab walk every row.", "Tab và Shift+Tab đi hết các hàng.", "Tab と Shift+Tab で各行を移動。", "Tab 和 Shift+Tab 走遍每一行。"),
...
const AR: &[(&str, &str)] = &[
    ...
    ("field.command", "أمر"),
    ("field.form-hint", "Tab و Shift+Tab يمشيان على كل صف."),
...
const UR: &[(&str, &str)] = &[
    ...
    ("field.command", "کمانڈ"),
    ("field.form-hint", "Tab اور Shift+Tab ہر قطار چلتے ہیں۔"),
...
const HE: &[(&str, &str)] = &[
    ...
    ("look.dark", "ערכת צבע כהה"),
    ("look.light", "ערכת צבע בהירה"),
    ...
    ("job.list-detail", "רשימה ליד לוח פירוט. בחרו שורה; הימין הוא אותה שורה."),
    ("job.inspector", "בחרו קובץ. האמצע הוא המסמך. העמודה הימנית היא מאפיינים."),
    ("job.workspace", "פיצול עורך. העגינה נועצת את המתאר כלח השלישי. גררו את המפריד."),
    ("job.navigation", "מקומות בצד ההתחלה. חלונות צרים נערמים עם חזרה."),
    ...
    ("wjob.workspace", "פיצול עורך: קבצים בצד ההתחלה, עריכה ומסוף כלשוניות. גררו את המפריד."),
    ("wjob.drawer", "חלונית צד שמתחבא. סגור מצייר רק את התוכן."),
    ("wjob.tool-panel", "לחצו עגינה כדי לנעוץ מתאר זה כלח בצד הסוף של סביבת העבודה למעלה."),
    ("wjob.inspector", "בחרו קובץ. האמצע הוא המסמך. הימין הוא שם, סוג ונתיב."),
    ...
    ("wjob.dialogs", "ראשי וביטול אופציונלי. בוחרי קבצים מקומיים עוברים ב-native_dialog."),
    ...
    ("about.blurb", "פקדים ומסגרת ליישומי שולחן עבודה של iced 0.14."),
    ...
    ("field.focus-hint", "Enter שולח. העבר מיקוד נכנס לשם."),
    ...
    ("keys.hint", "הקלידו אות, או Enter, Escape, חץ, או מקש פונקציה."),
    ...
    ("motion.bounce-body", "bounce_out קופץ בנחיתה."),
    ("motion.pulse", "פעימה"),
    ...
    ("field.command", "פקודה"),
    ("field.form-hint", "Tab ו-Shift+Tab עוברים בכל שורה."),
    ...
    ("crumb.home", "בית"),
    ("crumb.gallery", "גלריה"),
    ("vc.hint", "כרטיסי הרחבה דרך virtual_column (הרכבת תצוגה)."),
...
const MARKDOWN_HE: &str = r#"# Markdown
